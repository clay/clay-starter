...
DS.controller('youtube', ['$gtm', function ($gtm) {
  function Constructor(el) {
    ...
    this.el = el;
    this.visible = new $visibility.Visible(this.el, { preloadThreshold: 800 });

    // when the video player element enters the viewport, load the video(s)
    if (this.visible.preload && $visibility.isElementNotHidden(el)) {
      ...
      this.visible.on('preload', function () {
        youtubeVideoPlayer.init(videoConfig);
      });
    }
